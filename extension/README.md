...
5. Click the popup icon to check backend URL settings. The backend defaults to the live deployment `https://mailscope-api.onrender.com` (no local server needed). The full-investigation web app URL is blank by default because no public frontend is deployed yet — see `shared/config.js`. For local development, set the backend field to `http://127.0.0.1:8000`.
